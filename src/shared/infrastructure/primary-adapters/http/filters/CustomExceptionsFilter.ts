...
export class CustomExceptionsFilter implements ExceptionFilter {
  private readonly logger = new Logger(CustomExceptionsFilter.name);
    ...
    if (exception instanceof HttpException) {
      ...
      if (typeof exResponse === 'object' && exResponse !== null) {
        ...
        if (
          'message' in exResponseObj &&
          Array.isArray(exResponseObj['message'])
        ) {
          body = {
          ...
          typeof exResponseObj === 'object' &&
          'code' in exResponseObj &&
          'message' in exResponseObj
        ) {
          body = {
          ...
          body = {
            code: 'HTTP_ERROR',
            message: 'Unexpected HTTP exception payload',
          };
        }
      } else {
  ...
  private mapErrorToStatus(error: Error): number {
    const statusMap = new Map([
      ...
      ['AccountTemporarilyLockedError', HttpStatus.TOO_MANY_REQUESTS],
      ...
      ['InvalidTradeExecutionDateError', HttpStatus.BAD_REQUEST],
