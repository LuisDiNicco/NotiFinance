...
export class TypeOrmNotificationRepository implements INotificationRepository {
    ...
    public async findByUserPaginated(userId: string, unreadOnly: boolean, page: number, limit: number): Promise<Notification[]> {
    ...
    public async findById(notificationId: string): Promise<Notification | null> {
        const entity = await this.repository.findOne({ where: { id: notificationId } });
