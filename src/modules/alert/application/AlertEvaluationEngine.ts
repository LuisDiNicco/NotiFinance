...
export class AlertEvaluationEngine {
    ...
    public async evaluateAlertsForAsset(assetId: string, currentPrice: number): Promise<Alert[]> {
        const alerts = await this.alertRepository.findActiveByAssetId(assetId);
        return this.evaluateAndPersist(alerts, currentPrice);
    }

    public async evaluateAlertsForDollar(dollarType: string, currentPrice: number): Promise<Alert[]> {
        const alerts = await this.alertRepository.findActiveByType(AlertType.DOLLAR);
        const scoped = alerts.filter((alert) => !alert.period || alert.period === dollarType);
    ...
    private async evaluateAndPersist(alerts: Alert[], currentValue: number): Promise<Alert[]> {
