...
export class AlertService {
    ...
    public async createAlert(userId: string, alert: Alert): Promise<Alert> {
        ...
        if (alert.assetId) {
            ...
            if (!exists) {
                throw new Error(`Asset ${alert.assetId} not found`);
            }
        }
    ...
    public async getUserAlerts(userId: string, page = 1, limit = 20): Promise<Alert[]> {
        return this.alertRepository.findByUserIdPaginated(userId, page, limit);
    }

    public async updateAlert(userId: string, alertId: string, patch: Partial<Alert>): Promise<Alert> {
    ...
    public async changeStatus(userId: string, alertId: string, status: AlertStatus): Promise<Alert> {
