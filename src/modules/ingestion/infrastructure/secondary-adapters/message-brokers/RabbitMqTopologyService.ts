...
export class RabbitMqTopologyService implements OnModuleInit, OnModuleDestroy {
  ...
  async onModuleInit(): Promise<void> {
    const rabbitMqUrl = this.configService.get<string>(
      'integrations.rabbitmq.url',
    );
    if (!rabbitMqUrl) {
      this.logger.warn(
        'RABBITMQ_URL is missing. Skipping topology declaration.',
      );
      return;
    }
    ...
    await channel.assertExchange('notifinance.events', 'topic', {
      durable: true,
    });

    await channel.assertQueue('alert-evaluation-queue.dlq', { durable: true });
    await channel.assertQueue('notification-events-queue.dlq', {
      durable: true,
    });
    ...
    this.logger.log(
      'RabbitMQ topology declared: notifinance.events + alert/notification queues with DLQ',
    );
  }
