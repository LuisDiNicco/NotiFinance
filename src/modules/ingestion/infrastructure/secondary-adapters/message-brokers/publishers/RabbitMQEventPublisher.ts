...
export class RabbitMQEventPublisher implements IEventPublisher {
  ...
  async publishEvent(
    ...
    try {
      ...
      this.logger.debug(
        `Successfully emitted event to RabbitMQ Exchange: ${event.eventType}`,
      );
    } catch (error) {
      this.logger.error(
        `Failed to publish event ${event.eventId} to RabbitMQ`,
        error,
      );
      throw new Error(
        `Failed to publish event (RabbitMQ Publisher): ${(error as Error).message}`,
      );
    }
  }
}
