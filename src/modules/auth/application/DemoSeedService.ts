...
export class DemoSeedService {
    ...
    public async createDemoUserWithSeedData(): Promise<User> {
        ...
        const demoPasswordHash = await bcrypt.hash(randomUUID(), DemoSeedService.SALT_ROUNDS);
    ...
    private async seedPortfolio(userId: string): Promise<void> {
        ...
        if (!portfolio.id) {
            this.logger.warn(`Demo portfolio for user ${userId} has no id. Skipping trade seeding.`);
            return;
        }
        ...
        for (const trade of trades) {
            try {
                ...
                this.logger.warn(`Skipping demo trade for ${trade.ticker}: ${(error as Error).message}`);
            }
        }
    }

    private async seedWatchlist(userId: string): Promise<void> {
        const tickers = ['GGAL', 'YPFD', 'PAMP', 'BMA', 'AL30', 'GD30', 'AAPL', 'MSFT', 'GOOGL', 'NVDA'];

        for (const ticker of tickers) {
            try {
                await this.watchlistService.addToWatchlist(userId, ticker);
            } catch (error) {
                this.logger.warn(`Skipping demo watchlist ticker ${ticker}: ${(error as Error).message}`);
            }
        }
    }
    ...
    private async seedAlerts(userId: string): Promise<void> {
        ...
        try {
            ...
            this.logger.warn(`Skipping dollar demo alert: ${(error as Error).message}`);
        }
