...
export class YahooFinanceClient implements IQuoteProvider {
    public async fetchQuote(yahooTicker: string): Promise<MarketQuote> {
        const quote = await yahooFinance.quote(yahooTicker) as YahooQuoteLike;
        const now = new Date();
    ...
    public async fetchHistorical(yahooTicker: string, startDate: Date, endDate: Date): Promise<MarketQuote[]> {
