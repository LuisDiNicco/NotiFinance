...
export class TypeOrmQuoteRepository implements IQuoteRepository {
    ...
    public async saveBulkQuotes(quotes: MarketQuote[]): Promise<void> {
        const persistableQuotes = quotes.filter(
            (quote): quote is MarketQuote & { assetId: string } => typeof quote.assetId === 'string',
        );
    ...
    public async findByAssetAndPeriod(assetId: string, startDate: Date, endDate: Date): Promise<MarketQuote[]> {
    ...
    public async findTopMovers(
        ...
        const latestRows = await this.repository.query(
            `
        ...
        const sortedByChange = asDomain
            ...
            .sort((left, right) => (right.changePct as number) - (left.changePct as number));
