...
export class PortfolioService {
    ...
    public async getPortfolioHoldings(userId: string, portfolioId: string): Promise<Holding[]> {
        const portfolio = await this.getPortfolioDetail(userId, portfolioId);
        if (!portfolio?.id) {
            return [];
        }
        ...
        const assetsById = new Map<string, { ticker: string; currentPrice: number }>();
        for (const asset of assets) {
            ...
            let currentPrice = 0;
            try {
                const quotes = await this.marketDataService.getAssetQuotes(asset.ticker, 2);
        ...
        return this.holdingsCalculator.calculateHoldings(trades, priceMap, tickerMap);
    }

    public async getPortfolioDistribution(userId: string, portfolioId: string): Promise<Array<{ ticker: string; weight: number }>> {
