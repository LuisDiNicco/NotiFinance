...
export class TypeOrmWatchlistRepository implements IWatchlistRepository {
    ...
    public async findByUserId(userId: string): Promise<WatchlistItem[]> {
        ...
        return entities.map(WatchlistItemMapper.toDomain);
    }

    public async findByUserAndAsset(userId: string, assetId: string): Promise<WatchlistItem | null> {
        const entity = await this.repository.findOne({ where: { userId, assetId } });
