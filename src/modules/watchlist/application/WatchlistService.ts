...
import { WATCHLIST_REPOSITORY, type IWatchlistRepository } from './IWatchlistRepository';
...
export class WatchlistService {
    ...
    public async addToWatchlist(userId: string, ticker: string): Promise<WatchlistItem> {
        ...
        const existing = await this.watchlistRepository.findByUserAndAsset(userId, asset.id);
        if (existing) {
            return existing;
        }

        return this.watchlistRepository.save(new WatchlistItem({ userId, assetId: asset.id }));
    }

    public async removeFromWatchlist(userId: string, ticker: string): Promise<void> {
        const asset = await this.marketDataService.getAssetByTicker(ticker);
        if (!asset.id) {
            return;
        }
