...
import { EVENT_PUBLISHER, IEventPublisher } from '../src/modules/ingestion/application/IEventPublisher';
...
describe('Market -> Alert -> Notification flow (integration)', () => {
    ...
    const buildRmqContext = () => {
        ...
        return {
            ...
            context: {
                ...
                getMessage: () => ({ fields: {}, properties: {}, content: Buffer.from('') }),
    ...
    it('publishes market event, evaluates alert and dispatches notification', async () => {
        ...
        const publishedEvent = eventPublisher.publishEvent.mock.calls[0]?.[0] as EventPayload;
        const publishedCorrelationId = eventPublisher.publishEvent.mock.calls[0]?.[1];
        ...
        await eventConsumer.handleIncomingEvent(alertMessage, notificationContext.context);
