...
describe('Template security (e2e)', () => {
  let app: INestApplication;
  const previousTemplateAdminApiKey = process.env['TEMPLATE_ADMIN_API_KEY'];

  beforeAll(async () => {
    process.env['TEMPLATE_ADMIN_API_KEY'] = 'template-admin-key';
  ...
  it('/api/v1/templates (GET) keeps read access available', async () => {
    await request(app.getHttpServer())
      .get('/api/v1/templates?page=1&limit=10')
      .expect(200);
  });

  it('/api/v1/templates (POST) rejects unauthenticated writes', async () => {
    await request(app.getHttpServer())
  ...
  it('/api/v1/templates/test-compile (POST) rejects unauthenticated calls', async () => {
    await request(app.getHttpServer())
