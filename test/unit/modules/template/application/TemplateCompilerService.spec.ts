...
import {
  ITemplateRepository,
  TEMPLATE_REPO,
} from '../../../../../src/modules/template/application/ITemplateRepository';
...
describe('TemplateCompilerService', () => {
  ...
  it('should compile templates properly replacing nested and flat variables', async () => {
    repo.findByEventType.mockResolvedValue(
      new NotificationTemplate(
        ...
        'test.event',
        'Hello {{ user.name }}',
        'Your total is {{ amount }} with status {{status}}',
      ),
    );
  ...
  it('should render date placeholders as ISO strings', async () => {
    repo.findByEventType.mockResolvedValue(
      new NotificationTemplate(
        'Test',
        'test.event',
    ...
    const when = new Date('2026-01-01T12:34:56.000Z');
    const result = await service.compileTemplate('test.event', { when });

    expect(result.subject).toBe('At 2026-01-01T12:34:56.000Z');
    expect(result.body).toBe('Body 2026-01-01T12:34:56.000Z');
  });

  it('should render non-primitive object placeholders as empty string', async () => {
    repo.findByEventType.mockResolvedValue(
      new NotificationTemplate(
        'Test',
        'test.event',
